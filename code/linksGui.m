...
% Last Modified by GUIDE v2.5 06-May-2013 15:07:25
...
switch EventData.Character
    ...
    case 'f'
       ...
       [seli selj] = getNeiInd(j,i,31,512,512);
   ...
   case 's'
       ...
       [seli selj] = getNeiInd(j,i,20,512,512);
    ...
    case 'r' 
       ...
       dist = max(min([ i, 512-i,81,j,512-j]),9);
       
       [seli selj] = getNeiInd(j,i,dist,512,512);
...
function mouseMove(hObject,eventdata)
    ...
    if(dKey == 1 || aKey == 1)
        ...
        line_i = line_i + 1.2*randn(size(line_i));
        line_j = line_j + 1.2*randn(size(line_j));
        
        line_i = ceil(line_i);
        line_j = ceil(line_j);
        
        line_i(line_i<1) = 1; line_i(line_i>512) = 512;
        line_j(line_j<1) = 1; line_j(line_j>512) = 512;
                
        ind = sub2ind([512 512],line_j,line_i);
...
% handles    structure with handles and user data (see GUIDATA)
   ...
   global k a previousSeg moviePath;
   ...
   segPara.minSize= 1.2; 
   segPara.maxSize= 1.8;
   segPara.thresh = str2double( get(handles.edit3,'String'));
   segPara.openSize = 4.0;
      
   doDraw = 0;
   generateFilters;
   filters = filters(:,:,1:9:60);
      
   [cout b] = segmentSingleImage(sub_seg,segPara,filters,openFilter,0);
